...
San supplies one JSON value on stdin and closes it, so scripts that read until
EOF finish deterministically. Command hooks run without the TUI's controlling
terminal.
