...
description: >-
  ...
  first, ordered by severity, then fixes on request. This is the bug-hunting
  counterpart to simplify (which covers reuse, quality, and efficiency, not bugs).
  Use when the user says "code review", "review my changes", "any bugs", "find
  bugs", or asks whether a change is correct.
allowed-tools:
  - Bash
  - Read
  - Glob
...
scoped to bugs. Reuse, quality, and efficiency cleanups belong to the `simplify`
skill; do not report those here.
