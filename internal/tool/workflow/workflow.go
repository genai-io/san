...
func (t *WorkflowTool) Schema() core.ToolSchema {
	return core.ToolSchema{
		Name: t.Name(),
		Description: "Run a workflow: several subagent turns arranged as a graph, in the background, reporting one summary when done. " +
			...
			"The definition is markdown. Optional frontmatter sets name and max_parallel (default 4). One ```mermaid block gives the topology: " +
			"`a --> b` runs b after a with a's output; `a --> b & c` fans out; `a -->|HIGH| b` runs b only when a's trimmed output equals HIGH. " +
...
// PreparePermission validates the whole definition and resolves every node's
// agent before the user is asked, so a workflow that cannot run is refused
// rather than approved. Nothing is carried over to Execute: every stage of a
// tool call parses its own copy of the arguments, so a value stashed on this
// map would be dropped on the floor.
func (t *WorkflowTool) PreparePermission(_ context.Context, params map[string]any, _ string) (*perm.PermissionRequest, error) {
	...
	definition, err := tool.RequireString(params, "definition")
	if err != nil {
		return nil, err
	}
...
// describe is the approval title: name, size, and what the run is for. The
// approval view has no workflow preview yet, so this one line is what the
// user approves; the definition itself is in the tool_use block.
func describe(w *workflow.Workflow, description string) string {
	...
	s := fmt.Sprintf("Run workflow %s: %d nodes, max %d parallel", name, len(w.Nodes), w.MaxParallel)
	if description != "" {
		s += " — " + description
	}
	return s
...
func (t *WorkflowTool) Execute(_ context.Context, params map[string]any, _ string) toolresult.ToolResult {
	...
	w, err := workflow.Parse(definition)
	if err != nil {
		return toolresult.NewErrorResult(t.Name(), fmt.Sprintf("invalid workflow:\n%v", err))
	}
...
func (r *nodeRunner) RunNode(ctx context.Context, n *workflow.Node, prompt string) (string, error) {
	res, err := r.exec.Run(ctx, tool.AgentExecRequest{
		Agent:       n.Config["agent"],
		Prompt:      prompt,
		Description: r.name + "/" + n.ID,
		Background:  true,
		Model:       n.Config["model"],
		Mode:        n.Config["mode"],
		OnActivity:  func(msg string) { r.task.AppendProgress(n.ID + " ▸ " + msg) },
	})
	if err != nil {
		return "", err
	}
