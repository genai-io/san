...
// Tool holds state for the tool selector overlay.
type Tool struct {
	active        bool
	tools         []toolItem
...
// NewTool creates a new Tool with injected load/save callbacks.
func NewTool(
	loadDisabled func(userLevel bool) map[string]bool,
	saveDisabled func(disabled map[string]bool, userLevel bool) error,
) Tool {
...
func (s *Tool) EnterSelect(width, height int, disabledTools map[string]bool, mcpTools func() []core.ToolSchema) error {
...
func (s *Tool) HandleKeypress(key tea.KeyMsg) tea.Cmd {
	if key.String() == "tab" {
