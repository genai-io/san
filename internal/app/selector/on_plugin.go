...
// Plugin holds state for the plugin selector
type Plugin struct {
...
func NewPlugin(reg *coreplugin.Registry) Plugin {
	cwd, _ := os.Getwd()
	return Plugin{
		registry:           reg,
		active:             false,
...
func UpdatePlugin(rt Runtime, state *Plugin, msg tea.Msg) (tea.Cmd, bool) {
	switch msg := msg.(type) {
	...
	case PluginUninstallMsg:
		...
		if uninstalled {
			_ = rt.ReloadAfterPluginChange()
		}
		return nil, true

	case PluginInstallMsg:
		return tea.Batch(pluginInstallCmd(state.registry, rt.Cwd, msg), state.startLoadingTick()), true
...
func (s *Plugin) beginLoading(msg string) tea.Cmd {
	s.isLoading = true
	s.loadingMsg = msg
...
func (s *Plugin) startLoadingTick() tea.Cmd {
	if s.loadingTicking {
		return nil
	}
...
func (s *Plugin) HandleKeypress(key tea.KeyMsg) tea.Cmd {
...
func (s *Plugin) handleAddMarketplaceKeypress(key tea.KeyMsg) tea.Cmd {
	switch key.String() {
	case "esc":
		s.goBack()
		return nil
...
func (s *Plugin) handleDetailKeypress(key tea.KeyMsg) tea.Cmd {
...
func (s *Plugin) handleBrowseKeypress(key tea.KeyMsg) tea.Cmd {
	if s.handleNavigationKey(key, true) {
		return nil
	}
	switch key.String() {
...
func (s *Plugin) handleNavigationKey(key tea.KeyMsg, vimKeys bool) bool {
	switch key.String() {
	case "up", "ctrl+p":
		s.MoveUp()
		return true
...
func (s *Plugin) handleListKeypress(key tea.KeyMsg) tea.Cmd {
	if s.searchQuery == "" {
		switch key.String() {
		case "tab", "right":
			s.NextTab()
...
func (s *Plugin) handleListRuneKey(r string) tea.Cmd {
	if s.searchQuery == "" {
		switch r {
		case "l":
			s.enterDetail()
...
func (s *Plugin) handleMarketplaceAction(action func(pluginMarketplaceItem) tea.Cmd) tea.Cmd {
...
// Tab navigation
func (s *Plugin) NextTab() { s.switchTab((s.activeTab + 1) % 3) }
func (s *Plugin) PrevTab() { s.switchTab((s.activeTab + 2) % 3) }

func (s *Plugin) switchTab(tab pluginTab) {
	s.activeTab = tab
	s.resetListState()
	s.resetDetailState()
	s.resetBrowseState()
...
func (s *Plugin) filterItemsForTab(query string) []any {
	switch s.activeTab {
...
func (s *Plugin) enterInstalledDetail() {
...
func (s *Plugin) enterDiscoverDetail() {
	if s.selectedIdx >= len(s.filteredItems) {
		return
	}
...
func (s *Plugin) enterMarketplaceDetail() {
	if s.selectedIdx == 0 {
...
func (s *Plugin) buildInstalledActions(p pluginItem) []pluginAction {
	actions := []pluginAction{}
	if p.Enabled {
...
func (s *Plugin) buildDiscoverActions(p pluginDiscoverItem) []pluginAction {
	actions := []pluginAction{}
	if !p.Installed {
		actions = append(actions,
...
func (s *Plugin) buildMarketplaceActions(m pluginMarketplaceItem) []pluginAction {
	return []pluginAction{
...
func (s *Plugin) executeAction() tea.Cmd {
...
func (s *Plugin) installPlugin(scope coreplugin.Scope) tea.Cmd {
	if s.detailDiscover == nil {
		return nil
	}
...
func (s *Plugin) browseMarketplace() {
	if s.detailMarketplace == nil {
		return
	}
...
func (s *Plugin) syncMarketplace(id string) tea.Cmd {
	s.isLoading = true
...
func (s *Plugin) toggleSelectedPlugin() tea.Cmd {
...
func (s *Plugin) HandleToggleResult(msg PluginToggleResultMsg) {
	s.isLoading = false
	s.loadingMsg = ""
	if msg.Success {
		past := "Disabled"
...
func (s *Plugin) HandleUninstall(name string) bool {
...
func (s *Plugin) HandleInstallResult(msg PluginInstallResultMsg) {
	s.isLoading = false
	s.loadingMsg = ""
	if !msg.Success {
...
func (s *Plugin) HandleMarketplaceSync(msg PluginMarketplaceSyncResultMsg) {
	s.isLoading = false
	s.loadingMsg = ""
	if !msg.Success {
...
func (s *Plugin) HandleMarketplaceRemove(id string) {
...
func (s *Plugin) setError(msg string) {
	s.lastMessage = msg
	s.isError = true
}

// setSuccess sets a success message.
func (s *Plugin) setSuccess(msg string) {
	s.lastMessage = msg
	s.isError = false
}

// clearMessage clears the status message.
func (s *Plugin) clearMessage() {
	s.lastMessage = ""
	s.isError = false
}
...
func (s *Plugin) EnterSelect(width, height int) error {
	s.active = true
	s.width = width
	s.height = height
	s.resetListState()
...
func (s *Plugin) refreshCurrentTab() {
	switch s.activeTab {
	case pluginTabInstalled:
...
func (s *Plugin) refreshInstalledPlugins() {
...
func (s *Plugin) refreshDiscoverPlugins() {
...
func (s *Plugin) refreshMarketplaces() {
...
func (s *Plugin) getInstalledNames() map[string]bool {
...
func (s *Plugin) newDiscoverItem(name, marketplaceID string, installed map[string]bool) pluginDiscoverItem {
...
func (s *Plugin) enrichDiscoverItem(item *pluginDiscoverItem) {
...
func (s *Plugin) refreshAndUpdateView() {
	s.refreshCurrentTab()
...
func (s *Plugin) refreshDetailView() {
	if s.detailPlugin == nil {
		return
	}
...
func (s *Plugin) addMarketplace() tea.Cmd {
...
func (s *Plugin) resetDetailState() {
	s.detailPlugin = nil
...
func (s *Plugin) resetBrowseState() {
	s.browseMarketplaceID = ""
	s.browsePlugins = nil
}

func (s *Plugin) resetInputState() {
	s.searchQuery = ""
	s.filteredItems = nil
	s.addMarketplaceInput = ""
	s.addDialogCursor = 0
...
func (s *Plugin) resetLoadingState() {
	s.isLoading = false
	s.loadingMsg = ""
	s.loadingFrame = 0
