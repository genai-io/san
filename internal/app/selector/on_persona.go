...
// app applies it (persist + hot-patch) via Runtime.SetActivePersona.
...
// Persona is the interactive /persona picker — a single-select list of
...
func NewPersona(reg *persona.Registry, settingSvc *setting.Settings) Persona {
	return Persona{registry: reg, settingSvc: settingSvc}
}
...
func (s *Persona) EnterSelect(width, height int) error {
	if s.registry == nil {
...
func (s *Persona) IsActive() bool { return s.active }

func (s *Persona) Cancel() {
	s.active = false
	s.confirmDelete = false
	s.items = nil
	s.selectedIdx = 0
...
func (s *Persona) selected() (personaItem, bool) {
...
func (s *Persona) HandleKeypress(key tea.KeyMsg) tea.Cmd {
...
func (s *Persona) contentWidth() int { return max(60, s.width-6) }
func (s *Persona) boxHeight() int    { return max(18, s.height-4) }
func (s *Persona) bodyHeight() int   { return max(6, s.boxHeight()-10) }

func (s *Persona) renderViewport(content string) string {
...
// app callbacks on Runtime and shows a status line.
func UpdatePersona(rt Runtime, state *Persona, msg tea.Msg) (tea.Cmd, bool) {
	switch msg := msg.(type) {
	case PersonaSelectedMsg:
		state.Cancel()
		if rt.SetActivePersona != nil {
			if err := rt.SetActivePersona(msg.Name); err != nil {
				token := rt.State.Provider.SetStatusMessage("Persona switch failed: " + err.Error())
		...
		token := rt.State.Provider.SetStatusMessage(status)
		return kit.StatusTimer(3*time.Second, token), true

	case PersonaOpenMsg:
		if rt.OpenPersona != nil {
			return rt.OpenPersona(msg.Name), true
		}
		return nil, true

	case PersonaDeleteMsg:
		if rt.DeletePersona != nil {
			if err := rt.DeletePersona(msg.Name); err != nil {
				token := rt.State.Provider.SetStatusMessage("Delete failed: " + err.Error())
				return kit.StatusTimer(4*time.Second, token), true
			}
		}
		token := rt.State.Provider.SetStatusMessage("Deleted persona: " + msg.Name)
		return kit.StatusTimer(3*time.Second, token), true
	}
	return nil, false
}
