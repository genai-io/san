...
// MCP holds state for the MCP server selector
type MCP struct {
...
// NewMCP creates a new MCP with the given MCP registry.
func NewMCP(reg *coremcp.Registry) MCP {
	return MCP{
		registry: reg,
		active:   false,
		servers:  []mcpServerItem{},
...
func (s *MCP) EnterSelect(width, height int) error {
	if s.registry == nil {
...
func (s *MCP) AutoReconnect() tea.Cmd {
	var cmds []tea.Cmd
...
func (s *MCP) buildActions(srv mcpServerItem) []mcpAction {
...
func (s *MCP) executeAction() tea.Cmd {
...
func (s *MCP) HandleConnectResult(msg MCPConnectResultMsg) {
	s.connecting = false
	if msg.Success {
		s.lastError = ""
...
func (s *MCP) HandleDisconnect(name string) {
	if s.registry != nil {
...
func (s *MCP) HandleReconnect(name string) {
	if s.registry != nil {
...
func (s *MCP) HandleRemove(name string) {
	if s.registry != nil {
...
func (s *MCP) refreshAndUpdateView() {
	s.refreshServers()
...
func (s *MCP) AutoConnect() tea.Cmd {
	if s.registry == nil {
		return nil
	}
	var cmds []tea.Cmd
...
func UpdateMCP(rt Runtime, state *MCPState, msg tea.Msg) (tea.Cmd, bool) {
	switch msg := msg.(type) {
	case MCPConnectMsg:
	...
	case MCPConnectResultMsg:
		...
		if !state.Selector.IsActive() && !msg.Success {
			...
			rt.Conv.Append(core.ChatMessage{Role: core.RoleNotice, Content: content})
	...
	case MCPEditServerMsg:
		...
		if err != nil {
			rt.Conv.Append(core.ChatMessage{Role: core.RoleNotice, Content: fmt.Sprintf("Error: %v", err)})
			return tea.Batch(rt.CommitMessages()...), true
		}
	...
	case MCPEditorFinishedMsg:
		...
		if msg.Err != nil {
			...
			rt.Conv.Append(core.ChatMessage{Role: core.RoleNotice, Content: fmt.Sprintf("Editor error: %v", msg.Err)})
			return tea.Batch(rt.CommitMessages()...), true
		}

		if err := coremcp.ApplyServerEdit(state.Selector.registry, info); err != nil {
			rt.Conv.Append(core.ChatMessage{Role: core.RoleNotice, Content: fmt.Sprintf("Failed to apply edit: %v", err)})
			return tea.Batch(rt.CommitMessages()...), true
		}

		rt.Conv.Append(core.ChatMessage{Role: core.RoleNotice, Content: fmt.Sprintf("Updated MCP server '%s'", info.ServerName)})
		return tea.Batch(rt.CommitMessages()...), true
	}
	return nil, false
}
...
func (s *MCP) HandleKeypress(key tea.KeyMsg) tea.Cmd {
...
func (s *MCP) handleDetailKeypress(key tea.KeyMsg) tea.Cmd {
	switch key.String() {
	case "up", "ctrl+p":
		s.MoveUp()
		return nil
...
func (s *MCP) handleListKeypress(key tea.KeyMsg) tea.Cmd {
