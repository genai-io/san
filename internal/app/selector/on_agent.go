...
// Agent holds the state for the agent selector overlay.
type Agent struct {
...
func NewAgent(reg AgentRegistry) Agent {
	return Agent{
		registry:  reg,
		agents:    []agentItem{},
...
func (s *Agent) EnterSelect(width, height int) error {
...
func (s *Agent) IsActive() bool { return s.active }
...
func (s *Agent) tabCount(tab agentTab) int {
	count := 0
...
func (s *Agent) firstNonEmptyTab() agentTab {
...
func (s *Agent) saveLevelForActiveTab() bool {
...
func (s *Agent) cycleTab(delta int) {
...
func (s *Agent) HandleKeypress(key tea.KeyMsg) tea.Cmd {
	switch key.String() {
	case "tab", "right":
		s.cycleTab(+1)
		return nil
...
func (s *Agent) renderTabs() string {
	tabs := []kit.PanelTab{
...
func (s *Agent) renderEmpty() string {
	if len(s.agents) == 0 {
...
func (s *Agent) renderItemList(sb *strings.Builder, panel kit.Panel) {
...
func (s *Agent) renderHints() string {
	return kit.HintLine(
		"↑/↓ navigate",
		"Enter toggle",
		"←/→/Tab switch tab",
