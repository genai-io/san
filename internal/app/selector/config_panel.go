// /config popup shell. Config frames one or more sub-panels with
...
// Panel and append it to the panels slice in NewConfig.
package selector
...
// Config is the /config popup. Self-Learning and Appearance are
// registered today; adding a sibling panel (Provider, Permissions, …) is a
// one-liner in NewConfig.
type Config struct {
	panels []Panel
	index  int
...
// NewConfig wires the popup with its set of panels. Order is
// preserved by the tab strip.
func NewConfig(settings *setting.Settings) Config {
	return Config{
...
func (c *Config) Enter(width, height int) {
	c.width = width
	c.height = height
	c.active = true
...
func (c *Config) IsActive() bool { return c.active }
...
func (c *Config) HandleKeypress(msg tea.KeyMsg) tea.Cmd {
	if !c.active {
		return nil
	}
	switch msg.String() {
...
func (c *Config) ActivePanel() Panel { return c.activePanel() }

func (c *Config) activePanel() Panel {
	if len(c.panels) == 0 {
		return nil
	}
...
func (c *Config) renderHeader(width int) string {
	var left string
	if len(c.panels) == 1 {
...
func (c *Config) renderHint(panelHint string) string {
	parts := []string{}
	if panelHint != "" {
