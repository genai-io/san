...
func (s *Provider) Enter(ctx context.Context, width, height int) (tea.Cmd, error) {
	s.resetNavigation()
	s.resetModelSearch()
...
func (s *Provider) loadProviderData() (tea.Cmd, error) {
...
func (s *Provider) ensureModelProvidersExist() {
...
func (s *Provider) loadModelsAsync(store *llm.Store, current *llm.CurrentModelInfo) tea.Cmd {
...
func (s *Provider) HandleModelsLoaded(msg ProviderModelsLoadedMsg) {
...
func (s *Provider) loadModelsCached(allCached map[string][]llm.ModelInfo, current *llm.CurrentModelInfo) {
...
func (s *Provider) sortConnectedProviders(current *llm.CurrentModelInfo) {
	if current == nil {
		return
	}
...
func (s *Provider) rebuildVisibleItems() {
	s.visibleItems = nil
...
func (s *Provider) rebuildModelsTab() {
	s.updateFilter()
...
func (s *Provider) rebuildProvidersTab() {
...
func (s *Provider) clampSelection() {
...
func (s *Provider) resetConnectionResult() {
	s.lastConnectResult = ""
	s.lastConnectAuthIdx = 0
	s.lastConnectSuccess = false
}

func (s *Provider) resetModelSearch() {
	s.searchQuery = ""
	s.filteredModels = nil
	s.scrollOffset = 0
}

func (s *Provider) resetNavigation() {
	s.selectedIdx = 0
	s.scrollOffset = 0
