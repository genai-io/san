...
// Memory holds the state for the memory kit.
type Memory struct {
	active      bool
	items       []memoryItem
	selectedIdx int
...
// NewMemory creates a new memory selector.
...
func (m *Memory) EnterSelect(cwd string, width, height int) {
	m.cwd = cwd
	m.width = width
	m.height = height
	m.active = true
...
func (m *Memory) buildMemoryItem(label, level string, searchPaths []string, cwd, defaultDesc, createHint string) memoryItem {
...
func (m *Memory) HandleKeypress(key tea.KeyMsg) tea.Cmd {
	keyStr := key.String()
...
func (m *Memory) selectMemoryItem() tea.Cmd {
...
func (m *Memory) cancelMemoryWithMsg() tea.Cmd {
	m.Cancel()
	return func() tea.Msg {
...
func UpdateMemory(rt Runtime, state *MemoryState, msg tea.Msg) (tea.Cmd, bool) {
	switch msg := msg.(type) {
...
func handleMemorySelected(rt Runtime, state *MemoryState, msg MemorySelectedMsg) tea.Cmd {
	filePath := msg.Path

	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		if err := CreateMemoryFile(filePath, msg.Level, rt.Cwd); err != nil {
			rt.Conv.Append(core.ChatMessage{
	...
	displayPath := FormatMemoryDisplayPath(filePath, msg.Level, rt.Cwd)
...
func handleMemoryEditorFinished(rt Runtime, state *MemoryState, msg MemoryEditorFinishedMsg) tea.Cmd {
	...
	if msg.Err != nil {
		...
		rt.RefreshMemoryContext(rt.Cwd, "memory_edit")
...
func HandleMemoryCommand(selector *Memory, cwd string, width, height int, args string) (string, string, error) {
	args = strings.TrimSpace(args)
	parts := strings.Fields(args)
